...
public:
    TopologySortDFS(Graph &g): g(g) {
        ...
        this->res.resize(g.getV());
        std::fill(this->res.begin(), this->res.end(), 0);
...
int main() {
    ...
    TopologySortDFS topologySortDfs(g);
    vector<int> res = topologySortDfs.getRes();
    stringstream fmt;
    for (int v : res) {
        fmt << to_string(v) << ", ";
    }
